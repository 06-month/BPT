...
CONFIG = {
    ...
    "checkpoint": "rtmpose-l_coco.pth",
    "mim_config": "rtmpose-l_8xb256-420e_coco-256x192",
}


def parse_args():
    parser = argparse.ArgumentParser(description="Prepare RTMPose-l COCO 256x192 config/checkpoint.")
...
def main():
    ...
    source = find_installed_config(CONFIG["filename"])
    if source is None:
    ...
    if checkpoint is None and not args.skip_mim_download:
        checkpoint = try_mim_download(CONFIG)
    if checkpoint is None:
        ...
        print(f"  mim download mmpose --config {CONFIG['mim_config']} --dest {MODEL_DIR}")
...
def try_mim_download(config):
    if shutil.which("mim") is None:
        print("mim_status=missing")
        return None
    command = ["mim", "download", "mmpose", "--config", config["mim_config"], "--dest", str(MODEL_DIR)]
    print("mim_status=available")
    print(f"mim_command={' '.join(command)}")
    before = {path.resolve() for path in MODEL_DIR.glob("*.pth")}
    completed = subprocess.run(command, cwd=str(ROOT), check=False, capture_output=True, text=True, timeout=240)
    ...
    matches = sorted(MODEL_DIR.glob("rtmpose-l*.pth"), key=lambda path: path.stat().st_mtime, reverse=True)
