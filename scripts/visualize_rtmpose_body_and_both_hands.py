...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Visualize RTMPose-s body and both MediaPipe hand crops.",
    )
    ...
    parser.add_argument(
        "--output-2d",
        default="assets/smoke/rtmpose_body_both_hands_overlay.jpg",
    )
    parser.add_argument(
        "--output-left-3d",
        default="assets/smoke/rtmpose_left_hand_world_3d.png",
    )
    parser.add_argument(
        "--output-right-3d",
        default="assets/smoke/rtmpose_right_hand_world_3d.png",
    )
...
def main():
    ...
    try:
        with legacy_openmmlab_checkpoint_load():
            runner = RTMPoseRunner(
                pose_config=args.rtmpose_config,
                pose_checkpoint=args.rtmpose_checkpoint,
                device=args.device,
            )
        ...
        print({"body_detected": False, "rtmpose_runtime": "failed", "reason": str(exc)})
        return 0
    ...
    hand_runner, mediapipe_status = create_hand_runner()
...
def save_2d_overlay(image, keypoints, side_results, output_path, conf_threshold, sizes):
    import cv2

    overlay = image.copy()
    draw_body(overlay, keypoints, conf_threshold, sizes)
    for side, color in (("left", (255, 255, 0)), ("right", (255, 0, 255))):
        draw_side_hand(overlay, side_results[side], color, sizes)
...
def draw_body(
    ...
    conf_threshold,
    sizes,
    hide_wrist_limbs=False,
    hide_keypoint_labels=False,
    hide_skeleton=False,
):
    import cv2
    ...
    if not hide_skeleton:
        for a, b in BODY_SKELETON:
            if hide_wrist_limbs and (a, b) in BODY_WRIST_LIMBS:
                continue
    ...
    for idx, name in enumerate(KEYPOINT_NAMES):
        ...
        if conf(keypoints, idx) < conf_threshold:
            cv2.circle(image, point, sizes["body_radius"], (145, 145, 145), 1)
            continue
        color = body_keypoint_color(idx)
        cv2.circle(image, point, sizes["body_radius"], color, -1)
        if not hide_keypoint_labels:
            cv2.putText(
                image,
                short_name(name),
                (point[0] + sizes["body_radius"] + 2, point[1] - 2),
...
def draw_side_hand(
    ...
    sizes,
    draw_crop=True,
    draw_debug_vectors=True,
):
    import cv2

    side_body = side_result["side_body"]
    crop_box = side_result["crop_box"]
    if crop_box is not None and draw_crop:
        ...
        cv2.rectangle(
            ...
            (x2, y2),
            crop_color,
            sizes["line_thickness"] + 1,
        )
        label = side_result.get("hand_crop_mode") or "crop"
        cv2.putText(
            image,
            f"{side_result['side']} {label}",
            (x1 + 3, max(12, y1 - 4)),
            cv2.FONT_HERSHEY_SIMPLEX,
            sizes["font_scale"],
            crop_color,
    ...
    if hand_result is None or hand_result["hand_landmarks"] is None or crop_box is None:
        return
    ...
    labels = {0: "MP wrist", 5: "MP index_mcp", 9: "MP middle_mcp", 17: "MP pinky_mcp"}
    for idx, label in labels.items():
        point = point_to_int(points[idx])
        cv2.putText(
            image,
            label,
            (point[0] + 4, point[1] - 4),
    ...
    if draw_debug_vectors:
        elbow = point_to_int(side_body["elbow_px"])
        rtmpose_wrist = point_to_int(side_body["wrist_px"])
...
def draw_text_summary(image, side_results, sizes):
    ...
    lines = [
        ...
        "left wrist_alignment_state: "
        f"{side_results['left']['estimate'].get('wrist_alignment_state')}",
        ...
        "right wrist_alignment_state: "
        f"{side_results['right']['estimate'].get('wrist_alignment_state')}",
    ]
    line_height = max(14, int(18 * sizes["font_scale"] / 0.45))
    width = min(image.shape[1] - 8, 430)
...
def compact_side_result(side_result):
    ...
    return {
        ...
        "crop_box": side_result["crop_box"],
        "rtmpose_wrist_px": point_to_tuple(side_result["side_body"]["wrist_px"]),
        "mediapipe_wrist_px": None if hand_result is None else point_to_tuple(hand_result["wrist_px"]),
        "bend_angle_2d": estimate["bend_angle_2d"],
        "bend_state": estimate["bend_state"],
        "wrist_alignment_state": estimate.get("wrist_alignment_state"),
    }
