...
"$BIN" \
  --input-video assets/smoke/vedio_1.mp4 \
  --output-jsonl assets/smoke/vedio_1_apple_vision_3d_pose.jsonl \
  --max-frames 240 \
  --stride 1

status=$?
if [[ "$status" -ne 0 ]]; then
  echo "Apple Vision 3D vedio_1 smoke failed with exit code $status."
